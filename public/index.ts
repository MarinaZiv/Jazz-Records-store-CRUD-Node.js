...
const shopIcon: any = document.querySelector(".material-icons");
...
async function handleSearch(e:any) {
  try {
    ...
    // console.log(input);
    const {data} = await axios.get(`/albums/search?input=${input}`);
    renderToStore(data);
  } catch (error) {
    console.error(error);
  }
...
async function handleSelectType(e:any) {
  e.preventDefault();
  try {
    const type = e.target.value;
    console.log(e.target.value);

    const {data} = await axios.get(`/albums/select-type?type=${type}`);
